...
if __name__ == "__main__":
    ...
    if args.exp == "test-eval":
        d1_tasks = ["MNIST"]
        d2_tasks = ["UniformNoise", "NormalNoise", "NIHChest"]
        d3_tasks = ["UniformNoise", "NormalNoise", "NIHChest"]
        method_tasks = [
            "prob_threshold/0",
        ]
    ########################################################
    ...
    for d1 in d1_tasks:
        ...
        for method in method_tasks:
            ...
            for d2 in d2_tasks:
                ...
                print(
                    "Performing %s on %s vs. %s"
                    % (
                        colored(method, "green"),
                        colored(d1, "blue"),
                        colored(d2, "red"),
                    )
                )
                ...
                if not ds2.is_compatible(ds1):
                    print(
                        "%s is not compatible with %s, skipping."
                ...
                if torch.ByteTensor(
                    [
                        has_done_before(method, d1, d2, d3)
                        or not ds_cache[d3].is_compatible(ds1)
                        or d2 == d3
                        for d3 in d3_tasks
                    ]
                ).all():
                ...
                for d3 in d3_tasks:
                    ...
                    if not ds3.is_compatible(ds1):
                        print(
                            "%s is not compatible with %s, skipping."
                            % (colored(ds3.name, "red"), colored(ds1.name, "red"))
                        )
                        continue
                    ...
                    print(
                        "Final test size: %d+%d=%d"
                        % (len(d1_test), len(d2_test), len(test_mixture))
                    )
    ...
    for i, (m, ds, dm, dt, mi, a_train, a_test) in enumerate(results):
        print(
            "%d\t%s\t%15s\t%-15s\t%.2f%% / %.2f%%"
            % (i, m, "%s-%s" % (ds, dm), dt, a_train * 100, a_test * 100)
        )
