...
class Residual_AE(nn.Module):
    def __init__(self, dims, max_channels=1024, depth=7, n_hidden=1024):
        ...
        for i in range(depth):
            ...
            spatial_sizes.append(
                (
                    math.floor(((spatial_sizes[-1][0] - 1) / 2) + 1),
                    math.floor(((spatial_sizes[-1][1] - 1) / 2) + 1),
                )
            )
    ...
    def _calculate_out_pad(
        self, spatial_sizes, stride=2, padding=1, kernel_size=3, d=1
    ):
        out_pad = []
        for i in reversed(range(1, len(spatial_sizes))):
            ...
            for j in range(len(current)):
                outputsize = (
                    (current[j] - 1) * stride - 2 * padding + (kernel_size - 1) * d + 1
                )
...
class Generic_VAE(Generic_AE):
    ...
    def forward(self, x):
        ...
        n_size = enc.size(0)
        mu, logvar = (
            self.fc_e_mu(enc.view(n_size, -1)),
            self.fc_e_std(enc.view(n_size, -1)),
        )
        self.last_mu = mu
...
class ALILikeAE(nn.Module):
    ...
    def forward(self, x, sigmoid=False):
        ...
        dec = self.decoder(enc)
        if sigmoid or self.default_sigmoid:
            dec = (F.tanh(dec) + 1) / 2.0  # It's actually TanhHHHHHH
        return dec
