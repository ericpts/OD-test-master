...
class KNNModel(nn.Module):
    ...
    def __init__(self, base_data, k=1):
        ...
        self.base_data = (
            base_data.half().cuda()
        )  # We probably have to rewrite this part of the code
    ...
    def forward(self, x, **kwargs):
        ...
        diffs = (
            base_norm.unsqueeze(0).expand(n_samples, ref_size)
            + x_norm.unsqueeze(1).expand(n_samples, ref_size)
            - 2 * x.matmul(base_data.t())
        )
        diffs.sqrt_().detach_()
...
class KNNSVM(ScoreSVM):
    ...
    def propose_H(self, dataset):
        ...
        with tqdm(
            total=n_data, disable=bool(os.environ.get("DISABLE_TQDM", False))
        ) as pbar:
        ...
        self.base_model = KNNModel(self.base_data, k=self.default_model).to(
            self.args.device
        )
...
class AEKNNSVM(ScoreSVM):
    ...
    def propose_H(self, dataset):
        ...
        if isinstance(self, BCEKNNSVM) or isinstance(self, MSEKNNSVM):
            ...
            base_model = Global.get_ref_autoencoder(dataset.name)[1]().to(
                self.args.device
            )
        ...
        all_loader = DataLoader(
            dataset, batch_size=self.args.batch_size, num_workers=1, pin_memory=True
        )

        n_data = len(dataset)
        n_dim = base_model.encode(
            dataset[0][0].to(self.args.device).unsqueeze(0)
        ).numel()
        ...
        with torch.set_grad_enabled(False):
            with tqdm(
                ...
                for i, (x, _) in enumerate(all_loader):
                    ...
                    pbar.update()
        # self.base_data = torch.cat([x.view(1, -1) for x,_ in dataset])
        self.base_model = AEKNNModel(
            base_model, self.base_data, k=self.default_model
        ).to(self.args.device)
        self.base_model.eval()
...
class SVKNNSVM(ScoreSVM):
    ...
    def propose_H(self, dataset):
        ...
        home_path = get_ref_model_path(
            self.args, base_model.__class__.__name__, dataset.name
        )
        ...
        n_data = len(dataset)
        n_dim = base_model.partial_forward(
            dataset[0][0].to(self.args.device).unsqueeze(0)
        ).numel()
        print("nHidden %d" % (n_dim))
        self.base_data = torch.zeros(n_data, n_dim, dtype=torch.float32)
        ...
        # self.base_data = torch.cat([x.view(1, -1) for x,_ in dataset])
        self.base_model = AEKNNModel(
            base_model, self.base_data, k=self.default_model, SV=True
        ).to(self.args.device)
        self.base_model.eval()
