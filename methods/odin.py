...
class ODINModelWrapper(AbstractModelWrapper):
    ...
    def __init__(self, base_model, epsilon=0.0012, temperature=1000):
        ...
        )  # initialize to prob=0.5 for faster convergence.

        # register params under H for storage.
        self.H.register_buffer("epsilon", torch.FloatTensor([epsilon]))
        self.H.register_buffer("temperature", torch.FloatTensor([1.0 / temperature]))
...
class ODIN(ProbabilityThreshold):
    ...
    def get_H_config(
        self, train_ds, valid_ds, will_train=True, epsilon=0.0012, temperature=1000
    ):
        ...
        # Set up the model
        model = ODINModelWrapper(
            self.base_model, epsilon=epsilon, temperature=temperature
        ).to(self.args.device)
        ...
        if self.args.force_train_h or not path.isfile(done_path):
            ...
            for i_eps, eps in enumerate(all_epsilons):
                for i_temp, temp in enumerate(all_temperatures):
                        ...
                        if hasattr(h_config.model, "H") and hasattr(
                            ...
                            if h_config.visualize:
                                h_config.logger.get_measure(
                                    "threshold"
                                ).visualize_all_epochs(trainer.visdom)
                    ...
                    print(
                        "Hyper-param check (%.2e, %.1f) in %.2fs" % (eps, temp, elapsed)
                    )
