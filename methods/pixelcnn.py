...
class PixelCNNModelWrapper(AbstractModelWrapper):
    ...
    def __init__(self, base_model):
        assert isinstance(
            base_model, PCNNModel.PixelCNN
        ), "We only support PixelCNN objects."
...
class PixelCNN(ProbabilityThreshold):
    ...
    def get_base_config(self, dataset):
        ...
        h_path = get_ref_model_path(
            self.args, model.__class__.__name__, dataset.name, suffix_str=model.netid
        )
    ...
    def get_H_config(self, dataset, will_train=True):
        ...
        config.model = model
        config.optim = optim.Adagrad(
            model.H.parameters(), lr=1e-1, weight_decay=1.0 / len(train_ds)
        )
