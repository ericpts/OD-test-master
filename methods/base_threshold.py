...
class ProbabilityThreshold(AbstractMethodInterface):
    ...
    def get_base_config(self, dataset):
        ...
        model = Global.get_ref_classifier(dataset.name)[self.default_model]().to(
            self.args.device
        )
                ...
                for viz_param in viz_params:
                    if hasattr(h_config.model, "H") and hasattr(
                        ...
                        if h_config.visualize:
                            h_config.logger.get_measure(viz_param).visualize_all_epochs(
                                trainer.visdom
                            )
    ...
    def test_H(self, dataset):
        ...
        with tqdm(
            ...
            for i, (image, label) in enumerate(dataset):
                ...
                correct += (
                    (
                        classification.detach().view(-1)
                        == target.detach().view(-1).long()
                    )
                    .float()
                    .view(-1)
                    .sum()
        ...
        print(
            "Final Test average accuracy %s, AUROC %s, AP %s"
            % (
                colored("%.4f%%" % (test_average_acc * 100), "red"),
                colored("%.4f%%" % (auroc * 100), "green"),
                colored("%.4f%%" % (auprc * 100), "blue"),
            )
        )
        return (
            test_average_acc.item(),
            auroc,
            auprc,
            fpr,
            tpr,
