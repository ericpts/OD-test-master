...
class DeepEnsembleLoss(nn.Module):
    def __init__(self, ensemble_network, epsilon=0.01):
        ...
        assert (
            ensemble_network.__class__ == DeepEnsembleWrapper
        ), "Only EnsembleWrappers are accepted."
    ...
    def forward(self, input, target):
        ...
        if self.ensemble_network.training:
            ...
            grad_input_x = autograd.grad(
                [loss_1], [input_x], retain_graph=True, only_inputs=True
            )[0]

            # construct X' - Fast Gradient Sign Method + Projection
            new_input = (input_x.detach() + 0.01 * grad_input_x.detach().sign()).clamp(
                min=0, max=1
            )
            new_input.detach_()
...
class DeepEnsembleMasterWrapper(nn.Module):
    ...
    def forward(self, x, take_log=True, **kwargs):
        ...
        for model in self.subwrappers:
            model.eval()
            predictions = (
                model(x).unsqueeze(0).detach().exp()
        ...
        if take_log:
            return (
                output.log()
            )  # take the log for consistency with other models after averaging.
        else:
            return output
...
class DeepEnsemble(ProbabilityThreshold):
    ...
    def get_base_config(self, dataset):
        print(
            "Preparing training D1 for %s" % (dataset.parent_dataset.__class__.__name__)
        )
        ...
        for mid in range(5):
            ...
            h_path = get_ref_model_path(
                self.args, model_class.__name__, dataset.name, suffix_str="DE.%d" % mid
            )
