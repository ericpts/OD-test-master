...
if __name__ == "__main__":
    ...
    for method in methods:
        ...
        if not all(
            [has_done_before(method, "NIHCC", composite_D2.name, d3) for d3 in d3s]
        ):
    ...
    d3s = ["PADChestAP", "PADChestL", "PADChestAPHorizontal", "PADChestPED"]
    D3s = []
    for d3 in d3s:
        dataset = Global.all_datasets[d3]
        if "dataset_path" in dataset.__dict__:
        ...
        for d2, D2 in zip(d3s, D3s):
            ...
            for d3, D3 in zip(d3s, D3s):
                ...
                if not has_done_before(method, "NIHCC", d2, d3):
                    ...
                    test_results = eval_subroutine(mt, D1, D3)
                    results["results"].append(
                        [method, "NIHCC", d2, d3, mt.method_identifier(), trainval_acc]
                        + list(test_results)
                    )
                    torch.save(results, results_path)
