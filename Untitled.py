...
class PCXRayDataset(Dataset):
    def __init__(
        ...
        if self.duplicate:
            self.to_duplicate = np.random.choice(
                range(self.total_samples), self.total_samples // 2
            )
    ...
    def __getitem__(self, idx):
        ...
        sample["sample_weight"] = torch.max(
            sample["encoded_labels"] * self.labels_weights
        )
        ...
        if self.counter_examples:
            ...
            if self.duplicate:
                ...
                if idx in self.to_duplicate:
                    new_encoded_labels[: self.nb_classes // 2] = sample[
                        "encoded_labels"
                    ]
                else:
                    new_encoded_labels[self.nb_classes // 2 :] = sample[
            ...
            # pick a query label
            topresent = np.random.choice(
                range(len(sample["encoded_labels"])), p=self.labels_weights_dup
            )
    ...
    def _build_labels(self):
        ...
        self.labels_weights = torch.from_numpy(
            np.array([(len(self) / label) for label in labels_count], dtype=np.float32)
        )
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        if "PA" in sample:
            pa_img = sample["PA"]
            pa_img = 2 * (pa_img / 65536) - 1.0
            pa_img = pa_img.astype(np.float32)
            sample["PA"] = pa_img

        if "L" in sample:
            l_img = sample["L"]
            l_img = 2 * (l_img / 65536) - 1.0
