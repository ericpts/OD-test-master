...
from datasets.NIH_Chest import (
    NIHChestBinaryTrainSplit,
    NIHChestBinaryValSplit,
    NIHChestBinaryTestSplit,
)
...
def train_subroutine(ODmethod, D1, D2):
    ...
    print(
        "Adjusting %s and %s to %s"
        % (
            colored("D1=%d" % d1_valid_len, "red"),
    ...
    print(
        "Final valid size: %d+%d=%d"
        % (len(d1_valid), len(d2_valid), len(valid_mixture))
    )
...
def eval_subroutine(ODmethod, D1, D3):
    ...
    acc, auroc, auprc, fpr, tpr, precision, recall, TP, TN, FP, FN = ODmethod.test_H(
        test_mixture
    )
...
if __name__ == "__main__":
    ...
    D1 = PADChestBinaryTrainSplit(
        root_path=os.path.join(args.root_path, "PADChest"), binary=True
    )
    D164 = PADChestBinaryTrainSplit(
        root_path=os.path.join(args.root_path, "PADChest"), binary=True, downsample=64
    )
    args.D1 = "PADChest"
    ...
    for d2 in d2s:
        ...
        if "dataset_path" in dataset.__dict__:
            ...
            D2s.append(
                dataset(root_path=os.path.join(args.root_path, dataset.dataset_path))
            )
        else:
            D2s.append(dataset())
    composite_D2 = D2s[0]
    ...
    for method in methods:
        ...
        if not all(
            [has_done_before(method, "PADChestL", composite_D2.name, d3) for d3 in d3s]
        ):
    ...
    d3s = ["PADChestAP", "PADChestPA", "PADChestAPHorizontal", "PADChestPED"]
    D3s = []
    for d3 in d3s:
        dataset = Global.all_datasets[d3]
        if "dataset_path" in dataset.__dict__:
    ...
    for method in methods:
        print("current method", method)
        for d2 in d3_tags:
            if not has_done_before(
                method, "PADChestL", "PADChestL_" + d2, "PADChestL_test"
            ):
                mt = Global.get_method(method, args)
    ...
    ) in enumerate(results["results"]):
        print(
            "%d\t%s\t%15s\t%-15s\t%.2f%% / %.2f%% - %.2f%%"
            % (
                i,
                m,
                "%s-%s" % (ds, dm),
