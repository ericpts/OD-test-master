...
def process_results(d2_handles, results):
    ...
    for acc, roc, prc in zip(__acc, __auroc, __auprc):
        try:
            ...
            uc_acc[acc[1], acc[2], acc[3]] = uc_acc[acc[1], acc[2], acc[3]] * (
                weights[acc[1], acc[2], acc[3]] - 1
            ) / weights[acc[1], acc[2], acc[3]] + acc[0] / (
                weights[acc[1], acc[2], acc[3]]
            )
            uc_roc[roc[1], roc[2], roc[3]] = uc_roc[roc[1], roc[2], roc[3]] * (
                weights[roc[1], roc[2], roc[3]] - 1
            ) / weights[roc[1], roc[2], roc[3]] + roc[0] / (
                weights[roc[1], roc[2], roc[3]]
            )
            uc_prc[prc[1], prc[2], prc[3]] = uc_prc[prc[1], prc[2], prc[3]] * (
                weights[prc[1], prc[2], prc[3]] - 1
            ) / weights[prc[1], prc[2], prc[3]] + prc[0] / (
    ...
    return (
        np.stack([weights, uc_acc, uc_roc, uc_prc]),
        [method_handles, true_d2_handles, d3_handles],
    )
...
def make_plot(filename, title, true_d2_handles, method_handles, csv_data):
    ...
    rects3 = ax.bar(ind + width * 0.75, uc1_prcm, width, yerr=uc1_prcv, label="AUPRC")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--result_dir",
        type=str,
        default="workspace/experiments/padchest_res_raw",
        help="path to folder of result_n.pth files",
    )
    parser.add_argument(
        "--cache_dir",
        type=str,
        help="load cached results from here (precedes results path when specified)",
    )
    ...
    for i, d2tags in zip(range(1, 4, 1), d2sets):
        if args.cache_dir is None:
            ...
            np.save(
                os.path.join(args.output_dir, "data_UC%d_%s.npy" % (i, args.dataset)),
                csv_data,
            )
            with open(
                os.path.join(
                    args.output_dir, "headers_UC%d_%s.pkl" % (i, args.dataset)
                ),
                "wb",
            ) as fp:
                    ...
                    for d3 in range(csv_data.shape[3]):
                        ...
                        csv_list.append(
                            [
                                ...
                                csv_data[1, m, d2, d3] * 100.0,  # acc
                                csv_data[2, m, d2, d3] * 100.0,  # auc
                                csv_data[3, m, d2, d3] * 100.0,  # prc
                            ]
                        )
            with open(
                os.path.join(args.output_dir, "data_UC%d_%s.csv" % (i, args.dataset)),
                "w+",
            ) as fp:
            ...
            assert os.path.isfile(
                os.path.join(args.cache_dir, "data_UC%d_%s.npy" % (i, args.dataset))
            )
            csv_data = np.load(
                os.path.join(args.cache_dir, "data_UC%d_%s.npy" % (i, args.dataset))
            )
            assert os.path.isfile(
                os.path.join(args.cache_dir, "headers_UC%d_%s.pkl" % (i, args.dataset))
            )
            with open(
                os.path.join(args.cache_dir, "headers_UC%d_%s.pkl" % (i, args.dataset)),
                "rb",
