...
class IDCBase(data.Dataset):
    ...
    def extract(self):
        if os.path.exists(os.path.join(self.source_dir, self.image_dir)):
            return
        with ZipFile(
            os.path.join(self.source_dir, "IDC_regular_ps50_idx5.zip"), "r"
        ) as zipObj:
    ...
    def generate_index(self):
        ...
        for (dirpath, dirnames, filenames) in os.walk(
            os.path.join(self.source_dir, self.image_dir)
        ):
...
class IDC(AbstractDomainInterface):
    ...
    def __init__(
        self,
        root_path="./workspace/datasets/IDC",
        downsample=None,
