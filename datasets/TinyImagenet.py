...
class TinyImagenetParent(data.Dataset):
    ...
    def _check_exists_raw(self):
        return os.path.exists(
            os.path.join(self.root, self.raw_folder, "tiny-imagenet-200.zip")
        )
    ...
    def extract(self):
        ...
        if not os.path.exists(index_path):
            ...
            valid_set = []
            with open(
                os.path.join(output_path, "val", "val_annotations.txt"), "r"
            ) as f:
                for row in f:
                    row = row.split("\t")
                    valid_set.append(
                        (
                            os.path.join(output_path, "val", "images", row[0]),
                            class_to_idx[row[1]],
                        )
                    )
...
class TinyImagenet(AbstractDomainInterface):
    ...
    def __init__(
        ...
        if self.downsample is None:
            ...
            im_transformer = transforms.Compose(
                transform_list
                + [
                    transforms.Resize((self.downsample, self.downsample)),
                    transforms.ToTensor(),
                ]
            )
        self.ds_train = TinyImagenetParent(
            root_path,
            split="train",
        ...
        self.ds_valid = TinyImagenetParent(
            root_path,
            split="valid",
            transform=im_transformer,
            download=download,
        ...
        self.ds_test = TinyImagenetParent(
            root_path,
            split="test",
            transform=im_transformer,
            download=download,
    ...
    def get_D2_test(self, D1):
        ...
        if D1.name in self.filter_rules:
            target_indices = filter_indices(
                self.ds_valid, target_indices, self.filter_rules[D1.name]
            )
        return SubDataset(
            self.name,
            self.ds_valid,
