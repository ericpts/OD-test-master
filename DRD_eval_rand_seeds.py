...
if __name__ == "__main__":
    ...
    D1 = DRD(
        root_path=os.path.join(args.root_path, "diabetic-retinopathy-detection"),
        downsample=224,
    )
    D164 = DRD(
        root_path=os.path.join(args.root_path, "diabetic-retinopathy-detection"),
    ...
    for method in methods:
        ...
        if not all(
            [has_done_before(method, "DRD", composite_D2.name, d3) for d3 in d3s]
        ):
        ...
        for d2, D2 in zip(d3s, D3s):
            ...
            for d3, D3 in zip(d3s, D3s):
                ...
                if not has_done_before(method, "DRD", d2, d3):
                    ...
                    test_results = eval_subroutine(mt, D1, D3)
                    results["results"].append(
                        [method, "DRD", d2, d3, mt.method_identifier(), trainval_acc]
                        + list(test_results)
                    )
                    torch.save(results, results_path)
