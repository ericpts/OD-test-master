...
if __name__ == "__main__":
    dataset = PCAM(
        root_path=os.path.join(args.root_path, "pcam"), extract=True, downsample=64
    ).get_D1_train()
    dataloader = torch.utils.data.DataLoader(
        dataset, args.batch_size, True, num_workers=args.workers, pin_memory=True
    )
    ...
    if not os.path.isfile(hbest_path + ".done"):
        ...
        optimizerG = optim.Adam(
            [{"params": model.GenX.parameters()}, {"params": model.GenZ.parameters()}],
            lr=args.lr,
        ...
        for epoch in range(1, 100 + 1):
            ...
            with tqdm(
                ...
                for i, (x, y) in enumerate(dataloader):
                    ...
                    if args.random_label == True:
                        truelabel = (
                            torch.randint(low=70, high=110, size=(1, batchsize))[0]
                            / 100
                        )
                        fakelabel = (
                            torch.randint(low=-10, high=30, size=(1, batchsize))[0]
                            / 100
                        )
                    truelabel = truelabel.cuda()
                    fakelabel = fakelabel.cuda()
                    loss_d = criterion(pred_real.view(-1), truelabel) + criterion(
                        pred_fake.view(-1), fakelabel
                    )
                    loss_g = criterion(pred_fake.view(-1), truelabel) + criterion(
            ...
            print(
                "Discriminator loss %.4f, Generator loss %.4f" % (disc_loss, gen_loss)
            )
