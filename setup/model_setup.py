...
if __name__ == "__main__":
    ...
    task_list = [
        ...
        (
            Global.dataset_reference_classifiers,
            KLogisticSetup.train_classifier,
            True,
            ["KLogistic"],
        ),
    ...
    for task_id, (ref_list, train_func, skippable, suffix) in enumerate(task_list):
        ...
        for dataset in target_datasets:
            print(
                "Processing dataset %s with %d networks for %d-%s."
                % (
                    colored(dataset, "green"),
                    len(ref_list[dataset]),
                    task_id,
                    colored(train_func.__name__, "blue"),
                )
            )
            if skippable and not needs_processing(
                args, Global.all_datasets[dataset], ref_list[dataset], suffix=suffix
            ):
