...
def get_KLclassifier_config(args, model, home_path, dataset):
    ...
    config.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        config.optim, patience=10, threshold=1e-2, min_lr=1e-6, factor=0.1, verbose=True
    )
...
def train_classifier(args, model, dataset):
    ...
    print(
        "All average accuracy %s" % colored("%.4f%%" % (test_average_acc * 100), "red")
    )
