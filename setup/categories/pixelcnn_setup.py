    ...
    with torch.set_grad_enabled(False):
        for i in range(obs[1]):
            for j in range(obs[2]):
                ...
                out_sample = None
                if obs[0] == 1:
                    out_sample = pcnn_utils.sample_from_discretized_mix_logistic_1d(
                        out, nmix
                    )
                else:
                    out_sample = pcnn_utils.sample_from_discretized_mix_logistic(
                        out, nmix
                    )
...
def get_pcnn_config(args, model, home_path, dataset):
    ...
    valid_loader = DataLoader(
        valid_ds, batch_size=args.batch_size, num_workers=args.workers, pin_memory=True
    )
    ...
    config.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        config.optim, patience=10, threshold=1e-2, min_lr=1e-5, factor=0.1, verbose=True
    )
    config.max_epoch = 60
...
def train_pixelcnn(args, model, dataset):
    ...
    if not os.path.isfile(hbest_path + ".done"):
        ...
        for epoch in range(1, config.max_epoch + 1):
            ...
            lrs = [
                float(param_group["lr"]) for param_group in config.optim.param_groups
            ]
