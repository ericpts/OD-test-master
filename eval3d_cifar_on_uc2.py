...
if __name__ == "__main__":
    ...
    D1 = NIHChestBinaryTrainSplit(
        root_path=os.path.join(args.root_path, "NIHCC"), doubledownsample=32
    )
    ...
    D2 = Global.all_datasets["CIFAR10"](
        root_path=os.path.join(args.root_path, "cifar10")
    )
    args.D2 = "CIFAR10"
    d3s = [
        "PADChestAP",
        ...
        for d3, D3 in zip(d3s, D3s):
            if not has_done_before(method, "NIHCC", "CIFAR", d3):
                ...
                results["results"].append(
                    [method, "NIHCC", "CIFAR", d3, mt.method_identifier(), trainval_acc]
                    + list(test_results)
                )
